...
class Mail
{
    ...
    public function getGroupRecipients($group, bool $english, array &$recipients): void
    {
        if (! is_array($group)) {
            $recipients = array_merge(
                ...
                array_map(
                    ...
                    $this->users->getGroupRecipients($group, $english)
                )
            );
            return;
        }
        ...
        foreach ($group as $groupName) {
            $recipients = array_merge(
                ...
                array_map(
                    [$this, 'getEmailFromUser'],
                    $this->users->getGroupRecipients($groupName, $english)
                )
            );
        }
    }
    ...
    public function getMessageContent(string $layout, array $data): string
    {
        switch ($layout) {
            ...
            case 'nieuwsbrief':
                ...
                return view('email/newsletter', $data);
            default:
                return $data['content'];
        }
    }
    ...
    {
        $css = file_get_contents(APPPATH . '/Views/email/mail.css');
