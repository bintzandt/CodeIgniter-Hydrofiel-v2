...
?>
    ...
    <table class="table table-sm table-striped table-hover">
        ...
        <tbody>
        <?php
        foreach ($pages as $hoofd) { ?>
            <tr>
                <td class="clickable-row" data-href="/admin/pages/edit/<?= $hoofd->pageId ?>"
                    style="padding-right: 0;"><?= $hoofd->nameNL ?></td>
                <td class="d-xs-none"><?= get_status(
                        $hoofd->isAccessible,
                        $hoofd->isVisible,
                        $hoofd->requiresLogIn
                    ) ?></td>
                <td><?= form_open(''); ?>
                    ...
                    <button type="button" class="delete button--icon" data-pageName="<?= $hoofd->nameNL ?>"
                            data-pageId="<?= $hoofd->pageId ?>"><i class="fa fa-trash"></i></button>
                    <?= form_close() ?>
                </td>
            </tr>
            <?php
            if ($hoofd->subPages !== null) {
                foreach ($hoofd->subPages as $sub) { ?>
                    <tr>
                        <td class="clickable-row pl-4"
                            data-href="/admin/pages/edit/<?= $sub->pageId ?>"><?= $sub->nameNL ?></td>
                        <td class="d-xs-none"><?= get_status(
                                $sub->isAccessible,
                                $sub->isVisible,
                                $sub->requiresLogIn
                        ...
                        <td><?= form_open(''); ?>
                            ...
                            <button type="button" class="delete button--icon" data-pageName="<?= $sub->nameNL ?>"
                                    data-pageId="<?= $sub->pageId ?>"><i class="fa fa-trash"></i></button>
                            <?= form_close(); ?>
                        </td>
                    </tr>
                <?php
