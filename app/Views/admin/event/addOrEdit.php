...
<?php
if ($edit_mode && $event->eventId) { ?>
    <input type="hidden" name="eventId" value="<?= $event->eventId ?>">
<?php
} ?>
    <div class="form-group">
        <label for="nameNL">Naam (Nederlands)</label>
        <input type="text" class="form-control" name="nameNL" id="nameNL" required placeholder="Naam"
               value="<?= old('nameNL', $edit_mode ? $event->nameNL : '') ?>">
    </div>
    <div class="form-group">
        <label for="summernote">Omschrijving (Nederlands)</label>
        <textarea class="form-control input-block-level" id="summernote" name="descriptionNL"><?= old(
                'descriptionNL',
                $edit_mode ? $event->descriptionNL : ''
            ) ?></textarea>
    </div>
    <div class="form-group">
        <label for="nameEN">Naam (Engels)</label>
        <input type="text" class="form-control" name="nameEN" id="nameEN" required
               placeholder="Vul een naam in voor onze Engelstalige gebruikers"
               value="<?= old('nameEN', $edit_mode ? $event->nameEN : '') ?>">
    </div>
    <div class="form-group">
        <label for="engels">Omschrijving (Engels)</label>
        <textarea class="form-control input-block-level" id="engels" name="descriptionEN"><?= old(
                'descriptionEN',
                $edit_mode ? $event->descriptionEN : ''
            ) ?></textarea>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <div class="input-daterange input-group" id="datepicker">
            <input type="text" class="form-control-sm form-control flatpickr" name="from" id="from"
                   value="<?= old('from', $edit_mode ? $event->from->format('d-m-Y H:i') : '') ?>"/>
            <span class="input-group-addon">&nbsp;-&nbsp;</span>
            <input type="text" class="form-control-sm form-control flatpickr" name="until" id="until"
                   value="<?= old('until', $edit_mode ? $event->until->format('d-m-Y H:i') : '') ?>"/>
        </div>
    </div>
    <div class="form-group">
        ...
        <input type="text" class="form-control" name="link" id="link"
               ...
               value="<?= old('link', $edit_mode ? $event->link : '') ?>">
    </div>
    <div class="form-group">
        <label for="location">Locatie</label>
        <input type="text" class="form-control" name="location" id="location"
               placeholder="Locatie waar het evenement plaats gaat vinden"
               value="<?= old('location', $edit_mode ? $event->location : '') ?>">
    </div>
    <div class="form-group">
        <label for="inschrijven">Inschrijven mogelijk</label>
        <label><input required type="radio" name="needsRegistration" onchange="toggleInschrijf($(this).val())"
                      id="inschrijven"
                      value="1" <?= ($edit_mode && $event->needsRegistration) ? 'checked' : '' ?>>Ja</label>
        <label><input required type="radio" name="needsRegistration" onchange="toggleInschrijf($(this).val())"
                      value="0" <?= ($edit_mode && ! $event->needsRegistration) ? 'checked' : '' ?>>Nee</label>
    </div>
    <div class="form-group <?= ($edit_mode && $event->needsRegistration) ? '' : 'd-none' ?>" id="registrationDeadline">
        ...
        <div class="input-group date">
            <input type="text" class="form-control flatpickr" name="registrationDeadline" id="inschrijf" value="<?= old(
                'registrationDeadline',
                ($edit_mode && $event->needsRegistration) ? $event->registrationDeadline->format('d-m-Y H:i') : ''
    ...
    <div class="form-group <?= ($edit_mode && $event->needsRegistration) ? '' : 'd-none' ?>" id="cancellationDeadline">
        ...
        <div class="input-group date">
            <input type="text" class="form-control flatpickr" name="cancellationDeadline" id="afmeld" value="<?= old(
                'cancellationDeadline',
                ($edit_mode && $event->needsRegistration && $event->cancellationDeadline) ? $event->cancellationDeadline->format(
                    'd-m-Y H:i'
                ) : ''
            ) ?>"><span class="input-group-addon"><i class="glyphicon glyphicon-th"></i></span>
        </div>
    ...
    <div class="form-group">
        ...
        <label><input required type="radio" name="needsPayment" id="needsPayment"
                      value="1" <?= ($edit_mode && $event->needsPayment) ? 'checked' : '' ?>>Ja</label>
        <label><input required type="radio" name="needsPayment"
                      value="0" <?= ($edit_mode && ! $event->needsPayment) ? 'checked' : '' ?>>Nee</label>
    </div>
    <div class="form-group">
        <label for="maximumRegistrations">Maximum aantal aanmeldingen</label>
        <input type="number" value="<?= old('maximumRegistrations', $edit_mode ? $event->maximumRegistrations : 0) ?>"
               id="maximumRegistrations" name="maximumRegistrations" class="form-control" min="0">
    ...
    <div id="nszk" class="<?= ($edit_mode && $event->kind === 'nszk') ? '' : 'd-none' ?>">
        <div class="form-group">
            ...
            <div id="wrapper">
                <?php
                if ($edit_mode && $event->kind === 'nszk') { ?>
                    <div id="slag">
                        <?php
                        $strokes = json_decode($event->strokes);
